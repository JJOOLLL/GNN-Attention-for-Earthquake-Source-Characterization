...
class GraphNet(nn.Module):
    def __init__(self, N_sub=50, N_t=2048, f0=4, dropout_rate=0.15, activation="tanh", num_heads=8,struct_dim=64,
                 struct_nhead=4,
                 num_layers=2, theta = 1, K_neighbors=5):
        super().__init__()
    ...
    def forward(self, waveforms, coords, weights):
        ...
        # 根据台站坐标为每个样本分别构造有效台站的无向 KNN 图
        coords_flat = coords[:, :2, :, :].reshape(B, N_sub, 2)
        edge_indices = []
        for batch_idx in range(B):
            valid_nodes = torch.nonzero(mask[batch_idx] > 0, as_tuple=False).flatten()
            if valid_nodes.numel() <= 1:
                continue

            valid_coords = coords_flat[batch_idx, valid_nodes]
            distances = torch.cdist(valid_coords, valid_coords)
            distances.fill_diagonal_(float('inf'))
            k = min(self.K_neighbors, valid_nodes.numel() - 1)
            neighbors = torch.topk(distances, k, dim=-1, largest=False).indices

            sources = valid_nodes.repeat_interleave(k)
            targets = valid_nodes[neighbors.reshape(-1)]
            edges = torch.stack([sources, targets], dim=0)
            edges = torch.cat([edges, edges.flip(0)], dim=1)
            edge_indices.append(edges + batch_idx * N_sub)
        ...
        # GCN 负责台站间信息交互，其余部分沿用无消息传递＋最大池化配置
        x = self.gcn_conv(x.reshape(B * N_sub, 128), edge_index)
        x = x.reshape(B, N_sub, 128)
